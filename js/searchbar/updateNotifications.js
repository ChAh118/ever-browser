const UPDATE_URL = 'https://ever-browserbrowser.org/ever-browser/updates/latestVersion.json'
...
function getUpdateRandomNum () {
  /* the update JSON might indicate that the update is only available to a % of clients, in order to avoid notifying everyone to update to a new version until there's time to report bugs.
      Create a random number that is saved locally, and compare this to the indicated % to deterever-browsere if the update notification should be shown. */
...
function showUpdateNotification (text, input, inputFlags) {
  ...
  if (update) {
    ...
    /* initially, only show an update notification when no tabs are open, in order to ever-browserimize disruption */
