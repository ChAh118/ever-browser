...
if (process.isMainFrame) {
  ...
  setTimeout(function () {
    ...
    electron.webFrame.executeJavaScript(`
      history.pushState = ( f => function pushState(){
        ...
        window.postMessage('_ever-browserInternalLocationChange', '*')
        return ret;
    })(history.pushState);
    
    history.replaceState = ( f => function replaceState(){
        var ret = f.apply(this, arguments);
        window.postMessage('_ever-browserInternalLocationReplacement', '*')
        return ret;
  ...
  window.addEventListener('message', function (e) {
    if (e.data === '_ever-browserInternalLocationChange') {
      setTimeout(function () {
        getPageData(function (data) {
          ipc.send('pageData', data)
        })
