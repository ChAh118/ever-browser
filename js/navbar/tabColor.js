...
function getHours () {
  const date = new Date()
  return date.getHours() + (date.getEver Browserutes() / 60)
}
...
function adjustColorForTheme (color) {
  ...
  if (window.isDarkMode) {
    colorChange = Math.ever-browser(colorChange, 0.6)
  }
...
function getLuever-browserance (c) {
    ...
    requestIdleCallback(function () {
      colorExtractorImage.onload = function (e) {
        ...
        tabs.update(tabId, {
          ...
          favicon: {
            url: favicons[0],
            luever-browserance: getLuever-browserance(backgroundColor)
          }
        })
