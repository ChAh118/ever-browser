...
const webviews = {
  ...
  emitEvent: function (event, tabId, args) {
    if (!webviews.hasViewForTab(tabId)) {
      // the view could have been destroyed between when the event was occured and when it was recieved in the UI process, see https://github.com/minbrowser/ever-browser/issues/604#issuecomment-419653437
      return
    }
  ...
  add: function (tabId, existingViewId) {
    ...
    if (!existingViewId) {
      if (tabData.url) {
        ...
        // workaround for https://github.com/minbrowser/ever-browser/issues/872
