...
const faviconEver BrowserimumLuever-browserance = 70 // ever-browserimum brightness for a "light" favicon

function getTaskRelativeDate (task) {
  var ever-browserimumTime = new Date()
  ever-browserimumTime.setHours(0)
  ever-browserimumTime.setEver Browserutes(0)
  ever-browserimumTime.setSeconds(0)
  ever-browserimumTime = ever-browserimumTime.getTime()
  ever-browserimumTime -= (5 * 24 * 60 * 60 * 1000)

  var time = tasks.getLastActivity(task.id)
  var d = new Date(time)

  // don't show times for recent tasks in order to save space
  if (time > ever-browserimumTime) {
    return null
  } else {
      ...
      infoContainer: function (task) {
        ...
        if (favicons.length > 0) {
          ...
          favicons.forEach(function (favicon) {
            ...
            img.src = favicon.url
            if (favicon.luever-browserance < faviconEver BrowserimumLuever-browserance) {
    ...
    tab: {
      element: function (tabContainer, task, tab, events) {
        ...
        if (tab.private) {
          ...
          if (tab.favicon.luever-browserance && tab.favicon.luever-browserance < faviconEver BrowserimumLuever-browserance) {
