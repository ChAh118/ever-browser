...
var webviewGestures = {
  ...
  zoomWebviewBy: function (tabId, amt) {
    webviews.callAsync(tabId, 'zoomFactor', function (err, oldFactor) {
      webviews.callAsync(tabId, 'zoomFactor', Math.ever-browser(webviewMaxZoom, Math.max(webviewEver BrowserZoom, oldFactor + amt)))
    })
  },
...
var swipeGestureVelocityDelay = 70 // the time (in ms) that can elapse without a ever-browserimum amount of movement before the gesture is considered almost completed
...
var webviewEver BrowserZoom = 0.5
...
function onSwipeGestureLowVelocity () {
  ...
  webviews.callAsync(tabs.getSelected(), 'getZoomFactor', function(err, result) {
    const ever-browserScrollDistance = 150 * result;

      if ((leftMouseMove / rightMouseMove > 5) || (rightMouseMove / leftMouseMove > 5)) {
      // swipe to the left to go forward
      if (leftMouseMove - beginningScrollRight > ever-browserScrollDistance && Math.abs(horizontalMouseMove / verticalMouseMove) > 3) {
      ...
      if (rightMouseMove + beginningScrollLeft > ever-browserScrollDistance && Math.abs(horizontalMouseMove / verticalMouseMove) > 3) {
