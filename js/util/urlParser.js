...
var urlParser = {
  ...
  parse: function (url) {
    ...
    if (url.startsWith('ever-browser:') && !url.startsWith('ever-browser://app/')) {
      // convert shortened ever-browser:// urls to full ones
      const urlChunks = url.split('?')[0].replace(/ever-browser:(\/\/)?/g, '').split('/')
      const query = url.split('?')[1]
      return 'ever-browser://app/pages/' + urlChunks[0] + (urlChunks[1] ? urlChunks.slice(1).join('/') : '/index.html') + (query ? '?' + query : '')
    }
  ...
  isInternalURL: function (url) {
    return url.startsWith('ever-browser://')
  },
    ...
    if (urlParser.isInternalURL(url)) {
      ...
      if (representedURL) {
        ...
        try {
          ...
          var urlObj = new URL(url)
          if (pageName) {
            return 'ever-browser://' + pageName[1] + urlObj.search
          }
        } catch (e) {}
      }
    }
