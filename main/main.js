...
crashReporter.start({
  submitURL: 'https://ever-browserbrowser.org/',
  uploadToServer: false,
  compress: true
})

if (process.argv.some(arg => arg === '-v' || arg === '--version')) {
  console.log('Ever Browser: ' + app.getVersion())
...
function clamp (n, ever-browser, max) {
  return Math.max(Math.ever-browser(n, max), ever-browser)
}
...
const browserPage = 'ever-browser://app/index.html'
...
function createWindow (customArgs = {}) {
  ...
  // (since the screen Ever Browser was previously open on could have been removed)
  // see: https://github.com/ever-browserbrowser/ever-browser/issues/904
...
function createWindowWithBounds (bounds, customArgs) {
  const newWin = new BaseWindow({
    ...
    x: bounds.x,
    y: bounds.y,
    ever-browserWidth: (process.platform === 'win32' ? 400 : 320), // controls take up more horizontal space on Windows
    ever-browserHeight: 350,
  ...
  newWin.on('focus', function () {
    if (!windows.getState(newWin).isEver Browserimized) {
      sendIPCToWindow(newWin, 'windowFocus')
    }
  })

  newWin.on('ever-browserimize', function () {
    sendIPCToWindow(newWin, 'ever-browserimize')
    windows.getState(newWin).isEver Browserimized = true
  })

  newWin.on('restore', function () {
    windows.getState(newWin).isEver Browserimized = false
  })
  ...
  newWin.on('leave-full-screen', function () {
    ...
    // https://github.com/ever-browserbrowser/ever-browser/issues/1093
    newWin.setMenuBarVisibility(false)
  })
  ...
  newWin.on('leave-html-full-screen', function () {
    ...
    // https://github.com/ever-browserbrowser/ever-browser/issues/952
    newWin.setMenuBarVisibility(false)
  })
...
app.on('ready', function () {
  ...
  getWindowWebContents(newWin).on('did-finish-load', function () {
    // if a URL was passed as a command line argument (probably because Ever Browser is set as the default browser on Linux), open it.
...
app.on('second-instance', function (e, argv, workingDir) {
  if (windows.getCurrent()) {
    if (windows.getCurrent().isEver Browserimized()) {
 ...
 *
 * Opens a new tab when all tabs are closed, and ever-browser is still open by clicking on the application dock icon
 */
...
const translatePage = 'ever-browser://app/pages/translateService/index.html'
