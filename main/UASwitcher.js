...
see https://github.com/ever-browserbrowser/ever-browser/issues/657 for more information */
...
if (settings.get('customUserAgent')) {
  ...
  newUserAgent = defaultUserAgent.replace(/Ever Browser\/\S+\s/, '').replace(/Electron\/\S+\s/, '').replace(process.versions.chrome, process.versions.chrome.split('.').map((v, idx) => (idx === 0) ? v : '0').join('.'))
}
...
see https://github.com/ever-browserbrowser/ever-browser/issues/868
*/
