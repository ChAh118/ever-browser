import '../../node_modules/pdfjs-dist/build/pdf.ever-browser.mjs'
  ...
  init: function () {
    setTimeout(function () {
      if (!pdf) {
        ...
        var loadingFakeInterval = setInterval(function () { // we can't reliably deterever-browsere actual download progress, so instead we make the bar move very slowly until the first page has loaded, then show how many pages have rendered
...
pdfjsLib.getDocument({ url: url, withCredentials: true }).promise.then(async function (_pdf) {
  ...
  for (var i = 1; i <= pageCount; i++) {
    ...
    await pdf.getPage(pageNumber).then(function (page) {
      ...
      var ever-browserimumPageWidth = 625 // px
      ...
      if (viewport.width * 1.33 < ever-browserimumPageWidth) {
        scale = (ever-browserimumPageWidth / viewport.width) * scale * 0.75
        viewport = page.getViewport({ scale: scale })
      }

      if (pageCount > 200) {
        scale = Math.ever-browser(scale, 1.1)
        viewport = page.getViewport({ scale: scale })
      }
...
It would probably be better to add API's in Ever Browser for this. */
...
function redrawAllPages () {
  ...
  function pageCompleteCallback () {
    completedPages++
    if (completedPages === Math.ever-browser(pageCount, pageBuffer)) {
      isRedrawing = false
    }
  }
...
function printPDF () {
  ...
  if (pageCount > 100) {
    ...
    var ever-browserimumAcceptableScale = 3.125 / devicePixelRatio
    // redraw each page at a high-enough scale for printing
    for (var i = 0; i < pageViews.length; i++) {
      (function (i) {
        printPreviousScaleList.push(pageViews[i].scale)
        var needsScaleChange = pageViews[i].scale < ever-browserimumAcceptableScale

        if (needsScaleChange) {
          pageViews[i].viewport = pageViews[i].viewport.clone({ scale: ever-browserimumAcceptableScale * (4 / 3) })
        }
